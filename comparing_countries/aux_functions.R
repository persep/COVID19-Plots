    ...
    for(c in 1:length(countries)) {
      ...
      output_list[[c]] <-
        list("covid_data" =
             ...
             "population" = raw_data$popData2019[country_indexes[1]])
...
extracting_ECDC_covid19 <-
  function(source = paste0("https://www.ecdc.europa.eu/sites/",
    ...
    names(raw_data)[1] <- "dateRep" #¿POR QUÉ COJONES CAMBIAN LOS NOMBRES A LAS COLUMNAS?
    ...
    global_data <-
      list("global_data" = global_data,
           "global_population" = 
             sum(na.omit(raw_data$popData2019[first_indexes_countries])))
    ...
    for (i in 1:length(countries_by_cont)) {
      ...
      population <- sum(na.omit(raw_data$popData2019[first_indexes_countries]))
...
comparative_countries <- function(data, log = FALSE, n_hab = 1,
  ...
  for (i in 1:n_countries) {
    ...
    for (j in 1:length(data$filter_data[[i]]$covid_data$cum_cases)) {
      if (j < length(data$filter_data[[i]]$covid_data$cum_cases)) {
        if(data$filter_data[[i]]$covid_data$cum_cases[j] <
           data$filter_data[[i]]$covid_data$cum_cases[j + 1]) {
          
          data$filter_data[[i]]$covid_data$cum_cases[j] <-
            data$filter_data[[i]]$covid_data$cum_cases[j + 1] + 
            round((data$filter_data[[i]]$covid_data$cum_cases[j - 1] -
                     data$filter_data[[i]]$covid_data$cum_cases[j + 1])/2)
          
        }
      }
    }
    data$filter_data[[i]]$covid_data$cases <-
      rev(c(0, diff(rev(data$filter_data[[i]]$covid_data$cum_cases))))
  ...
  for (i in 1:length(data$filter_data)) {
    ...
    n_cases[i] <-  length(data$filter_data[[i]]$covid_data$cum_cases)
      # ifelse(aligned_cases & !log,
  ...
  if (plot_cases) {
    ...
    for (i in 1:length(data$filter_countries)) {
      ...
      variable <- data$filter_data[[i]]$covid_data$cases / rel_pop[i]
      if (log) { variable <- replace(log(variable), is.na(log(variable)) |
                                       is.infinite(log(variable)), 0)}
      if (i == 1) {
  ...
  if(plot_deaths) {
    for (i in 1:length(data$filter_countries)) {
      ...
      variable <- data$filter_data[[i]]$covid_data$deaths / rel_pop[i]
      if (log) { variable <- replace(log(variable), is.na(log(variable)) |
                                       is.infinite(log(variable)), 0)}
      if (i == 1) {
  ...
  if(plot_cum_cases) {
    ...
    for (i in 1:length(data$filter_countries)) {
      ...
      variable <- data$filter_data[[i]]$covid_data$cum_cases / rel_pop[i]
      if (log) { variable <- replace(log(variable), is.na(log(variable)) |
                                       is.infinite(log(variable)), 0)}
      if (i == 1) {
    ...
    # Layout details
    aux_title <- ifelse(log, "Cumulative log cases", "Cumulative cases")
    aux_title <- paste(aux_title, ifelse(is.logical(n_hab), "",
  ...
  if(plot_cum_deaths) {
    ...
    for (i in 1:length(data$filter_countries)) {
      ...
      variable <- data$filter_data[[i]]$covid_data$cum_deaths / rel_pop[i]
      if (log) { variable <- replace(log(variable), is.na(log(variable)) |
                                       is.infinite(log(variable)), 0)}
      if (i == 1) {
    ...
    # Layout details
    aux_title <- ifelse(log, "Cumulative log deaths", "Cumulative deaths")
    aux_title <- paste(aux_title, ifelse(is.logical(n_hab), "",
                                         paste("per each", n_hab, "hab.")))
    fig4 <- fig4 %>%
